...
export class UserRepository {
  ...
  async removeSingleWishlistItem(
    userId: string,
    productId: string,
    skuId: string,
  ): Promise<any> {
    const result = await this.userModel
      .updateOne({ _id: userId }, { $pull: { wishlist: { productId, skuId } } })
      .exec();
  ...
  async removeMultipleWishlistItems(
    userId: string,
    selectedItems: { productId: string; skuId: string }[],
  ): Promise<any> {
    const result = await this.userModel
      .updateOne(
        { _id: userId },
