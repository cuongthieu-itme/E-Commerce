...
export class ProductRepository {
  constructor(
    ...
    private readonly productModel: Model<Products & Document>,
    @InjectModel(License.name)
    private readonly licenseModel: Model<License & Document>,
  ) {}
  ...
  async incrementSkuRemainingStock(
    productId: string,
    skuId: string,
    qty?: number,
  ): Promise<void> {
  ...
  async decrementSkuRemainingStock(
    productId: string,
    skuId: string,
    quantity?: number,
  ): Promise<void> {
  ...
  async getWishlistItems(
    wishlist: { productId: string; skuId: string }[],
    ...
    const wishlistProducts = wishlist
      .map(({ productId, skuId }) => {
        ...
        const skuDetail = product.skuDetails.find(
